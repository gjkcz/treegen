...
void RostClanek(float r, float rp, float r2, float sklonx, float sklony, float sklonX1, float sklonY1, float sklonz, float Dens, float Uzk, float posunX, float posunY, float posunZ, float tU)
{
	...
	if (r2==0.f)
	{
		...
		{
			OrigoX=(cos(sklonx)* (cos((float)a+sklonz) * r + posunX));
			...
			Tvertices[vPoradi].Pos = XMFLOAT3(cos(tan(OrigoX/OrigoY)+sklonz)*OrigoX/cos(tan(OrigoX/OrigoY)),sin(tan(OrigoX/OrigoY)+sklonz)*OrigoX/cos(tan(OrigoX/OrigoY)), Zkoord + sin(sklonx)*(cos((float)a) * r + posunX) + sin(sklony)*(sin((float)a)) * r + posunZ);			
