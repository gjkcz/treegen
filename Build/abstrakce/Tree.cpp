...
SimpleVertex*  GenerateTree(int nVetvi, int vPoradi, int nTv)
{
	...
	pVs[0].r.r = GenerateRndm(777,999)/10.f;
	...
	while(!tpv.p_0.k)
	{
		...
		//pVs[0].r=tpv.p_0.rT;
		pVs[0].x = tpv.p_0.x;
	pVs[0].rT.rotace= pVs[0].r.rotace -GenerateRndm(2,2)*PI/180;
	pVs[0].rT.sklon =  pVs[0].r.sklon -GenerateRndm(90,90)*PI/180;//-89*PI/180;
