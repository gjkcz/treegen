...
SimpleVertex*  GenerateTree(int nVetvi, int vPoradi, int nTv)
{
	...
	pVs[0].r.r = GenerateRndm(111,999)/100.f;

	pVs[0].rT.rotace= -PI/3;
	pVs[0].rT.sklon = -PI/2;//-89*PI/180;
	pVs[0].rT.r = pVs[0].r.r;
	pVs[0].rT = pVs[0].r;
	//float mezera = Rust.r*0.2;//d
	//360...obvod 1 stupne..
	pVs[0].de =  30.f* PI/180;
	...
	for(int x = 0; x<1; x++)
	{
		for(int x = 0; x < n; x++) //x neni to co bz melo
		{
...
tuplParamsVetve RostVetev(paramsVetve pV)
{
	...
	for (float x = 0.f; x< pV.d/pV.m; x+=1.f)
	{
		...
		TempRust.r += (pV.rT.r -pV.r.r)/(pV.de/pV.m);
		TempRust.rotace += (pV.rT.rotace-pV.r.rotace)/(pV.de/pV.m);
		TempRust.sklon += (pV.rT.sklon - pV.r.sklon)/(pV.de/pV.m);
