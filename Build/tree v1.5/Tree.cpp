...
SimpleVertex*  GenerateTree(int nVetvi, int vPoradi, int nTv)
{
	...
	while(!tpv.p_0.k)
	{
		if(GenerateRndm(0,99) == 1 && !pVs[0].k)
			{
				pVs[0].k = true;
				//pVs[x] = pVs[x+1];
				tpv=RostVetev(pVs[0]);
		...
		//pVs[0].r=tpv.p_0.rT;
		pVs[0].x = tpv.p_0.x;
	pVs[0].rT.rotace= pVs[0].r.rotace -GenerateRndm(0,45)*PI/180;
	pVs[0].rT.sklon =  pVs[0].r.sklon -GenerateRndm(0,45)*PI/180;//-89*PI/180;
	pVs[0].d =GenerateRndm(333,999)/1.f;
	//per = 2*PI/ GenerateRndm(20,100);
	}
	//RostOsy();
...
tuplParamsVetve RostVetev(paramsVetve pV)
{
	...
	for (float x = 1.f; x< pV.d/pV.m; x+=1.f)
	{
		//mezera = (2*PI*TemppV.r.r)/90; //360...obvod 1 stupne..
		TempRust.r += (pV.rT.r -pV.r.r)/(pV.d/pV.m );
		TempRust.rotace +=  (50/(x))*(pV.rT.rotace - pV.r.rotace)/(pV.d/pV.m);
		TempRust.sklon +=  (50/(x))*(pV.rT.sklon - pV.r.sklon)/(pV.d/pV.m);
