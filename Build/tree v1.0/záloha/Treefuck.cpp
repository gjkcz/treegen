...
tuplParamsVetve RostRozdvojeni(XMFLOAT3 start,objRust Rust,float delka,float AxeRotace, float mezera, float dens);
...
SimpleVertex*  GenerateTree(int nVetvi, int vPoradi, int nTv)
{
	...
	Barva = XMFLOAT4(/*GenerateRndm(0,2)/*/0.f, /*GenerateRndm(0,2)/*/0.0f,/*GenerateRndm(0,2)/*/1.1f,1.f);
	...
	int p = 0;int rad = 7;int ckV = 0;int lux = 0;
	tuplParamsVetve* duplPms = new tuplParamsVetve[256];
	...
	pVs[0].r.r = GenerateRndm(333,333)/100.f;

	pVs[0].rT.rotace= -PI/2;
	pVs[0].rT.sklon = -PI/2;//-89*PI/180;
	pVs[0].rT.r = pVs[0].r.r;
	//pVs[0].rT = pVs[0].r;
	//float mezera = Rust.r*0.2;//d
	//360...obvod 1 stupne..
	pVs[0].de =  10.f* PI/180;
	pVs[0].m = (2*PI*pVs[0].r.r)/(360/(pVs[0].de/(PI/180)));
	per = 2*PI/30.f;
	//gnVetvi = nVetvi;
	//gnVetvi=10;
	...
	pVs[0].x.y = 0.0f;
	pVs[0].x.z = 0.0f;
	pVs[0].d = GenerateRndm(111,999)/10.f;			

	for(int x = 0; x<256; x++)
	{
		pVs[x].k = tPvs[x].k = false;
		pVs[x].x = tPvs[x].x = pVs[0].x;
		tPvs[x].aR = pVs[x].aR = pVs[0].aR;
		tPvs[x].m = pVs[x].m = pVs[0].m;
		tPvs[x].de = pVs[x].de = pVs[0].de;
	}
	...
	for(int x = 0; x<rad; x++)
	{
		...
		for(int x = ckV; x < n + ckV; x++)       
		{
			if(GenerateRndm(0,999) == 1 && !pVs[x+1].k)
			{
				pVs[x+1].k = true;
				//RostVetev(pVs[x]);
				//pVs[x] = pVs[x+1];
			...
			else if(!pVs[x].k)
			{
				...
				tPvs[x].r = pVs[x].r;
				pVs[x+1].x = tuplPosice.p_0.x;
				tPvs[x].x = tuplPosice.p_1.x;
				pVs[x+1].r = pVs[x].r;
				pVs[x+1].d = GenerateRndm(222,999)/10.f;
				pVs[x+1].rT.rotace=pVs[x].r.r- /*PI/2 +*/ GenerateRndm(20,120)*PI/180;
				if(GenerateRndm(0,2)==1)
					pVs[x+1].rT.sklon = tPvs[x].rT.sklon = -GenerateRndm(65,80)*PI/180;//0.2f*pow(gonx*PI/180,2);
				else
					pVs[x+1].rT.sklon = tPvs[x].rT.sklon = -GenerateRndm(10,80)*PI/180;//0.2f*pow(gonx*PI/180,2);
				pVs[x+1].rT.r = pVs[x].r.r - GenerateRndm(0,999)/1000.f;
				tPvs[x].rT.rotace =/*Rust.rotace*/ /*+ PI/2 +*/+ GenerateRndm(10,180)*PI/180;
				tPvs[x].rT.r = pVs[x].r.r - GenerateRndm(111,999)/1000.f;
				tPvs[x].d = GenerateRndm(222,999)/10.f;

				//pVs[x] = tuplPosice.p_0;//tady akorat x+1
		...
		for(int x =  ckV; x < p+ckV; x++)
			pVs[n+x] = tPvs[x];//a tady akorat +1, tu nultou uz mame
		ckV += n;
		n = p +n;//2p+(n-p), tedy celk pocet je sum of nssss, jovvvvvvvvv
		p=0;
	}
	...
	/*for(int x = 0; x<rad; x++)
	{
		...
		for(int x = ckV; x < n + ckV; x++)       
		{
			...
			else
			{
				...
				pVs[x+1].x = tuplPosice.p_0.x;//tady akorat x+1
				tPvs[x].x = tuplPosice.p_1.x;
				p++;
			}
	...
	/*for(int x = 0; x<rad; x++)
	{
		...
		for(int x = 0; x < n; x++)       
		{
			...
			{
				tuplPosice = RostVetev(pVs[lux]);
				lux++;
				pVs[x].x = tuplPosice.p_0.x;//tady akorat x+1
				tPvs[x] = tuplPosice.p_1;
				p++;
			}
		...
		//for(int x =  ckV; x < p + ckV ; x++)
			//pVs[n+x].x = tPvs[x].x;//a tady akorat +1, tu nultou uz mame
		n = p +n;//2p+(n-p), tedy celk pocet je sum of nssss, jovvvvvvvvv
		ckV += n;
		p=0;
	}*/
...
tuplParamsVetve RostVetev(paramsVetve pV)
{
	...
	//r-=0.5;
	gonx = OrigoZ;
	if(/*start.z < height && gnVetvi > 0 && start.z > 0.f &&*/!pV.k) //neni symetrické, chybí multithreading :D
	{
		//gnVetvi--;
		return RostRozdvojeni(pV.x,TempRust, pV.d,pV.aR, pV.m, pV.de);
	}
	else
	{
	}
...
tuplParamsVetve RostRozdvojeni(XMFLOAT3 start,objRust Rust,float delka,float AxeRotace, float mezera, float dens)
{
	...
	tupla.p_1.d=tupla.p_0.d=GenerateRndm(222,555)/10.f; 
	...
	afterY = sin(atan(OrigoZ/OrigoY)+Rust.sklon)*(OrigoY/cos(atan(OrigoZ/OrigoY)));
	posunZ = cos(atan(OrigoZ/OrigoY)+Rust.sklon)*(OrigoY/cos(atan(OrigoZ/OrigoY)))+start.z; 
	tupla.p_0.rT.rotace=Rust.rotace- /*PI/2 +*/ GenerateRndm(20,120)*PI/180;
	...
	if(GenerateRndm(0,2)==1)
		tupla.p_0.rT.sklon = -GenerateRndm(65,80)*PI/180;//0.2f*pow(gonx*PI/180,2);
	else
		tupla.p_0.rT.sklon = -GenerateRndm(10,80)*PI/180;//0.2f*pow(gonx*PI/180,2);
	tupla.p_0.rT.r =Rust.r - GenerateRndm(0,999)/1000.f;
	tupla.p_0.d = 0;
	...
	tupla.p_1.rT.sklon = Rust1.sklon;
	tupla.p_1.rT.rotace =/*Rust.rotace*/ /*+ PI/2 +*/+ GenerateRndm(10,180)*PI/180;
	tupla.p_1.rT.r = Rust.r - GenerateRndm(111,999)/1000.f;
	tupla.p_1.d = GenerateRndm(222,999)/10.f;
