...
void RostClanekRozdvojeni(float r, float rp, float r2, float radiusZ, float sklony, float sklonX1, float sklonY1, float sklonz, float Dens, float Uzk, float posunX, float posunY, float posunZ, float tU);
...
void RostRozdvojeni(XMFLOAT3 start,XMFLOAT3 ending,float r,float delka,float sklon, float rotace,float AxeRotace)//vraci end
{
	...
	for (float x = 0.f; x<= 0.f; x+=1.f)
	{
		...
		for (float x = 0.f; x<= 110.f; x+=1.f)
		{
			...
			posunZ = cos(atan(OrigoZ/OrigoY)+sklon)*(OrigoY/cos(atan(OrigoZ/OrigoY)));
			posunY = cos(atan(afterY/OrigoX)+rotace)*(OrigoX/cos(atan(afterY/OrigoX)));
			posunX = sin(atan(afterY/OrigoX)+rotace)*(OrigoX/cos(atan(afterY/OrigoX)))-(Uzk + cos(acos(Uzk/r))*rp); // vzcentrov8n9
			//cos(atan(OrigoY/OrigoX)+rotace)*(OrigoX/cos(atan(OrigoY/OrigoX)));
			//rotacey=acos(posunY/ample);
			...
			RostClanekRozdvojeni(r, rp, r2, radiusZ, sklony,sklonX1,sklonY1, sklonz, Dens, Uzk, posunX, posunY, posunZ, tU);
...
void RostClanekRozdvojeni(float r, float rp, float r2, float radiusZ, float sklony, float sklonX1, float sklonY1, float sklonz, float Dens, float Uzk, float posunX, float posunY, float posunZ, float tU)
{
	...
	for (float a = PI/2-(PI/2-acos(Uzk/r)); a<= PI*1.5f+(PI/2-acos(Uzk/r)); a+=Dens)																						//Pulkruh
	{
		...
		OrigoY=cos(sklony)* (sin((float)a)) * r ;
		Tvertices[vPoradi].Pos = XMFLOAT3(cos(atan(OrigoY/OrigoX)+sklonz)*(OrigoX/cos(atan(OrigoY/OrigoX))) + posunX,sin(atan(OrigoY/OrigoX)+sklonz)*(OrigoX/cos(atan(OrigoY/OrigoX)))+ posunY, Zkoord + sin(sklony)*(sin((float)a)) * r + posunZ);		
	...
	for (float a = PI+acos(Uzk/r); a<= 3*PI-acos(Uzk/r); a+=Dens)																						//Vysece2 + pulkruh2
	{
		...
		OrigoY=cos(sklony)* (sin((float)a)  * r2) /*+ PosunY + PosunY1*/ ;
		if(a<=2*PI)
		{


			Tvertices[vPoradi].Pos = XMFLOAT3(cos(atan(OrigoY/OrigoX)+sklonz)*sqrt(pow(OrigoX,2.f) + pow(OrigoY,2.f)) + posunX,sin(atan(OrigoY/OrigoX)+sklonz)*sqrt(pow(OrigoX,2.f) + pow(OrigoY,2.f))+ posunY,/*PosunX1+ posunX +*/ sin(sklony)*(sin((float)a)) * r2   + Zkoord + posunZ);
		...
		else
		{
			...
			Tvertices[vPoradi].Pos = XMFLOAT3(cos(atan(OrigoY/OrigoX)+sklonz)*sqrt(pow(OrigoX,2.f) + pow(OrigoY,2.f)) + posunX,sin(atan(OrigoY/OrigoX)+sklonz)*sqrt(pow(OrigoX,2.f) + pow(OrigoY,2.f))+ posunY,/*PosunX1+ posunX +*/ sin(sklony)*(sin(a-2*PI)) * r2   + Zkoord + posunZ);
