...
void Tree3::generujIndicieKruhuXY(int x, int y)
{
    if(!btriangleList) {
        ...
        for(int i = 0; i < druhStromu.rozliseniE; ++i) {
            generujIndicieElementu(t::testValec, i+zacatek); //2.1
            generujIndicieElementu(t::testValec, i+konec); //2.1
        }
        generujIndicieElementu(t::testValec, 0+zacatek);//2
        generujIndicieElementu(t::testValec, 0+konec);
    } else {
        ...
        for(int l = zacitOdclanku; l < pocetClankuKIndexovani+zacitOdclanku; ++l) {
            ...
            for(int i = pocatecniIndexX, n = pocatecniIndexY; i < pocatecniIndexX+druhStromu.rozliseniE-1; ++i, ++n) {
                generujIndicieElementu(t::testValec, 0+i, n);
                generujIndicieElementu(t::testValec, 1+i);

                generujIndicieElementu(t::testValec, n);
                generujIndicieElementu(t::testValec, n+1,  1+i);
            }
            int n = pocatecniIndexY+druhStromu.rozliseniE-1;
//        std::cout << "n je : "<<n << std::endl;
            int i = pocatecniIndexX+druhStromu.rozliseniE-1;
            generujIndicieElementu(t::testValec, 0+i, n);
            generujIndicieElementu(t::testValec, pocatecniIndexX);

            generujIndicieElementu(t::testValec, n);
            generujIndicieElementu(t::testValec, pocatecniIndexY, pocatecniIndexX);
        }
        int konecnyClanek = y;
    }
}
...
void Tree3::generujVrcholyVetve(const D3DXVECTOR3& pocatek, int kolikClanku, VlastnostiVetve& pV)
{
    if(pV.k)
        pV.rT.r = 2000.01f;
...
int Tree3::generujIndicieVetve(int cislo, int kolikClanku)
{
...
bool Tree3::generujElementyVetve( VlastnostiVetve& pV)
{
///* Triangle list */
...
bool Tree3::generujVykreslovaciDataVetvi()
{
...
void Tree3::vykresli(bool osvetlovat) const         // const fcion, so it cant change anything in our object
{
...
bool Tree3::vytvorBufferVrcholu()
{
    bool vysledek;
#ifdef TREEVERBOSE
//    std::cout << "alokejme nejprve nejake misto pro buffer..." << std::endl;
    ...
    try {
        int structSize = (druhStromu.element==t::usecka)?sizeof( VrcholPKB ):sizeof( VrcholBK );
        if( FAILED( pzarizeni->CreateVertexBuffer( pocetVrcholu * structSize,
            ...
            std::cout << "Pocet dosud vytvorenych stromu je: " << Tree3::pocetInstanciStromu << std::endl;
        } else {
            vysledek = true;
#ifdef TREEVERBOSE
...
bool Tree3::vytvorBufferIndicii()
{
    bool vysledek = true;
...
bool Tree3::uzamkniPoleDoBuffru()
{
    bool vysledek;
...
bool Tree3::odemkniVrcholyProCteni()
{
    bool vysledek;
...
bool Tree3::zkopirujVrcholyDoBuffru(const std::vector<VrcholBK>& _vrcholy)
{
    bool vysledek = true;
...
bool Tree3::zkopirujIndicieDoBuffru(const std::vector<long>& _indicie)
{
    bool vysledek = true;
